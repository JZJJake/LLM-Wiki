  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-44">
        ...
        {mounted && (
          <DropdownMenuItem onClick={() => set主题(theme === 'dark' ? 'light' : 'dark')}>
            {theme === 'dark' ? (
              <><Sun className="mr-2 h-4 w-4" />浅色 Mode</>
            ) : (
              <><Moon className="mr-2 h-4 w-4" />深色 Mode</>
            )}
          </DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
...
function 创建WikiDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>创建 wiki</DialogTitle>
        </DialogHeader>
        <input
          value={name}
          onChange={(e) => onNameChange(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && on创建()}
